...
- Pulls around 10 years of daily adjusted prices via `yfinance` (adjusted, so dividends and splits don't create fake jumps).
- Computes a **20-day and 50-day moving average** and goes long when the short MA is above the long MA, flat otherwise.
- **Lags the signal by one day** (`.shift(1)`) before applying it to returns which is the single most important step, since the signal comes from a day's closing price, which you couldn't have acted on until the day was over. Skipping this is lookahead bias, and it makes broken strategies look brilliant.
